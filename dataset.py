...
class DataSetLoader(LoaderCoordinator):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        # Columns names for ml_1m
        self.column_names = ['userId', 'itemId', 'rating', 'timestamp', 'gender']

        # Columns name for coco
        #self.column_names = ['id', 'itemId', 'userId', 'rating', 'timestamp', 'gender']
...
class DataSet(AbstractDataset):
    ...
    def __init__(self, config, data_tuple, side_information_data, *args, **kwargs):
        ...
        self.gender = self.dataframe_to_dict_gender(data_tuple[0])
        self.probabilities = self.probabilities_gender()
    ...
    def dataframe_to_dict_gender(self, data):
        gender = {}
        j = 0
        for i in self.items:
            sel_ = data[data['itemId'] == i]
            # The dictionary gender has keys that range from 1 to n_item, 
            # and its value is another dictionary with the director's gender 
            # as the value and the items' ids as the keys.
            gender[j] = {i: sel_['gender'].values[0]}
            j = j + 1

        return gender

    def probabilities_gender(self):
        probabilities = {}
        j = 0
        c = 2 # Parameter C
        p1 = 100 // (c + 1)
        ...
        # The value of the gender column should be either
        # 1(male), 2(female) or 'm' (male), 'f' (female)
        m = 2 if type(list(self.gender[0].values())[0]) is np.dtype(np.int64) else 'm'

        for g in self.gender.values():
            # Probabilities is a dictionary in which the probability that each item
            # will be extracted is represented as a value and the indices range
            # from 1 to n_item by key.
            probabilities[j] = p2 if list(g.values())[0] == m else p1
            j = j + 1
